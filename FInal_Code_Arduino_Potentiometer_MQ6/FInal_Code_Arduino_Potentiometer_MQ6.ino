...
const uint64_t pipeNum = 0x7878787878; //ADDRESS 
...
void loop() 
{
  ...
  int sensorValue1 = map(analogRead(A3),0,1023,0,255); //sensor 1


  //********===SENSOR 2 & DATA ACQUISITION AND CONVERSION===********//

  int sensorValue2 = map(analogRead(A4),0,1023,0,255); //sensor 2
...
//  int rawData2 = analogRead(A2); //sensor 2
  ...
  else if(rawData > 150 && rawData <= 200)
  {
    digitalWrite(2, LOW);
    digitalWrite(3, HIGH);
    digitalWrite(4, LOW);
